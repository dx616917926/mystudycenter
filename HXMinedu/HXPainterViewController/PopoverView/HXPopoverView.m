...
#import "HXPopoverView_Configuration.h"
#import <QuartzCore/QuartzCore.h>

#pragma mark - Implementation

@implementation HXPopoverView
...
+ (HXPopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view withText:(NSString *)text delegate:(id<PopoverViewDelegate>)delegate {
    HXPopoverView *popoverView = [[HXPopoverView alloc] initWithFrame:CGRectZero];
...
+ (HXPopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view withTitle:(NSString *)title withText:(NSString *)text delegate:(id<PopoverViewDelegate>)delegate {
    HXPopoverView *popoverView = [[HXPopoverView alloc] initWithFrame:CGRectZero];
...
+ (HXPopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view withViewArray:(NSArray *)viewArray delegate:(id<PopoverViewDelegate>)delegate {
    HXPopoverView *popoverView = [[HXPopoverView alloc] initWithFrame:CGRectZero];
...
+ (HXPopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view withTitle:(NSString *)title withViewArray:(NSArray *)viewArray delegate:(id<PopoverViewDelegate>)delegate {
    HXPopoverView *popoverView = [[HXPopoverView alloc] initWithFrame:CGRectZero];
...
+ (HXPopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view withStringArray:(NSArray *)stringArray delegate:(id<PopoverViewDelegate>)delegate {
    HXPopoverView *popoverView = [[HXPopoverView alloc] initWithFrame:CGRectZero];
...
+ (HXPopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view withTitle:(NSString *)title withStringArray:(NSArray *)stringArray delegate:(id<PopoverViewDelegate>)delegate {
    HXPopoverView *popoverView = [[HXPopoverView alloc] initWithFrame:CGRectZero];
...
+ (HXPopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view withStringArray:(NSArray *)stringArray withImageArray:(NSArray *)imageArray delegate:(id<PopoverViewDelegate>)delegate {
    HXPopoverView *popoverView = [[HXPopoverView alloc] initWithFrame:CGRectZero];
...
+ (HXPopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view withTitle:(NSString *)title withStringArray:(NSArray *)stringArray withImageArray:(NSArray *)imageArray delegate:(id<PopoverViewDelegate>)delegate {
    HXPopoverView *popoverView = [[HXPopoverView alloc] initWithFrame:CGRectZero];
...
+ (HXPopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view withTitle:(NSString *)title withContentView:(UIView *)cView delegate:(id<PopoverViewDelegate>)delegate {
    HXPopoverView *popoverView = [[HXPopoverView alloc] initWithFrame:CGRectZero];
...
+ (HXPopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view withContentView:(UIView *)cView delegate:(id<PopoverViewDelegate>)delegate {
    HXPopoverView *popoverView = [[HXPopoverView alloc] initWithFrame:CGRectZero];
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        showDividerRects = HX_kShowDividersBetweenViews;
    }
    return self;
}
...
{
    UIFont *font = HX_kTextFont;
    
    CGSize screenSize = [self screenSize];
    CGSize textSize = [text sizeWithFont:font constrainedToSize:CGSizeMake(screenSize.width - HX_kHorizontalMargin*4.f, 1000.f) lineBreakMode:UILineBreakModeWordWrap];
    ...
    textView.font = font;
    textView.textAlignment = HX_kTextAlignment;
    textView.textColor = HX_kTextColor;
    textView.text = text;
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withViewArray:(NSArray *)viewArray
{
    ...
    for (UIView *view in viewArray) {
        ...
        float padding = (i == viewArray.count-1) ? 0 : HX_kBoxPadding;
    ...
    if (HX_kShowDividersBetweenViews) {
    ...
    for (UIView *view in viewArray) {
        ...
        if (HX_kShowDividersBetweenViews && i != viewArray.count-1) {
            CGRect dividerRect = CGRectMake(view.frame.origin.x, floorf(view.frame.origin.y + view.frame.size.height + HX_kBoxPadding*0.5f), view.frame.size.width, 0.5f);
            
            [((NSMutableArray *)dividerRects) addObject:[NSValue valueWithCGRect:dividerRect]];
        }
        
        //Only add padding below the view if it's not the last item
        float padding = (i == viewArray.count-1) ? 0.f : HX_kBoxPadding;
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withTitle:(NSString *)title withViewArray:(NSArray *)viewArray
{
    ...
    float titleHeightOffset = (titleSize.height > 0.f ? HX_kBoxPadding : 0.f);
    
    float totalHeight = titleSize.height + titleHeightOffset + HX_kBoxPadding;
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withStringArray:(NSArray *)stringArray
{
    ...
    for (NSString *string in stringArray) {
        ...
        textButton.titleLabel.textAlignment = HX_kTextAlignment;
        textButton.titleLabel.textColor = HX_kTextColor;
        [textButton setTitle:string forState:UIControlStateNormal];
        textButton.layer.cornerRadius = 4.f;
        [textButton setTitleColor:HX_kTextColor forState:UIControlStateNormal];
        [textButton setTitleColor:HX_kTextHighlightColor forState:UIControlStateHighlighted];
...
- (NSMutableArray*) makeTempViewsWithStrings:(NSArray *)stringArray andImages:(NSArray *)imageArray
{
    ...
    for (int i = 0; i < stringArray.count; i++) {
        ...
        label.font = font;
        label.textAlignment = HX_kTextAlignment;
        label.textColor = HX_kTextColor;
        label.text = string;
        ...
        float containerHeight = label.frame.size.height + HX_kImageTopPadding + HX_kImageBottomPadding + imageView.frame.size.height;
        ...
        imageView.frame = CGRectMake(floorf(containerWidth*0.5f - imageView.frame.size.width*0.5f), HX_kImageTopPadding, imageView.frame.size.width, imageView.frame.size.height);
        label.frame = CGRectMake(floorf(containerWidth*0.5f - label.frame.size.width*0.5f), imageView.frame.size.height + HX_kImageBottomPadding + HX_kImageTopPadding, label.frame.size.width, label.frame.size.height);
...
-(void)setupLayout:(CGPoint)point inView:(UIView*)view
{
    ...
    if (arrowPoint.x + HX_kArrowHeight > topViewBounds.size.width - HX_kHorizontalMargin - HX_kBoxRadius - HX_kArrowHorizontalPadding) {//Too far to the right
        arrowPoint.x = topViewBounds.size.width - HX_kHorizontalMargin - HX_kBoxRadius - HX_kArrowHorizontalPadding - HX_kArrowHeight;
        //NSLog(@"Correcting Arrow Point because it's too far to the right");
    } else if (arrowPoint.x - HX_kArrowHeight < HX_kHorizontalMargin + HX_kBoxRadius + HX_kArrowHorizontalPadding) {//Too far to the left
        arrowPoint.x = HX_kHorizontalMargin + HX_kArrowHeight + HX_kBoxRadius + HX_kArrowHorizontalPadding;
    ...
    if (xOrigin < CGRectGetMinX(topViewBounds) + HX_kHorizontalMargin) {
        xOrigin = CGRectGetMinX(topViewBounds) + HX_kHorizontalMargin;
    } else if (xOrigin + boxWidth > CGRectGetMaxX(topViewBounds) - HX_kHorizontalMargin) {
        //Check to see if the positioning puts the box out of the window towards the left
        xOrigin = CGRectGetMaxX(topViewBounds) - HX_kHorizontalMargin - boxWidth;
    }

    float arrowHeight = HX_kArrowHeight;

    float topPadding = HX_kTopMargin;
...
- (void)drawRect:(CGRect)rect
{
    ...
    float radius = HX_kBoxRadius; //Radius of the curvature.
    
    float cpOffset = HX_kCPOffset; //Control Point Offset.  Modifies how "curved" the corners are.
    ...
    if (!above) {
        [popoverPath addLineToPoint:CGPointMake(arrowPoint.x - HX_kArrowHeight, yMin)];//left side
        [popoverPath addCurveToPoint:arrowPoint controlPoint1:CGPointMake(arrowPoint.x - HX_kArrowHeight + HX_kArrowCurvature, yMin) controlPoint2:arrowPoint];//actual arrow point
        [popoverPath addCurveToPoint:CGPointMake(arrowPoint.x + HX_kArrowHeight, yMin) controlPoint1:arrowPoint controlPoint2:CGPointMake(arrowPoint.x + HX_kArrowHeight - HX_kArrowCurvature, yMin)];//right side
    }
    ...
    if (above) {
        [popoverPath addLineToPoint:CGPointMake(arrowPoint.x + HX_kArrowHeight, yMax)];//right side
        [popoverPath addCurveToPoint:arrowPoint controlPoint1:CGPointMake(arrowPoint.x + HX_kArrowHeight - HX_kArrowCurvature, yMax) controlPoint2:arrowPoint];//arrow point
        [popoverPath addCurveToPoint:CGPointMake(arrowPoint.x - HX_kArrowHeight, yMax) controlPoint1:arrowPoint controlPoint2:CGPointMake(arrowPoint.x - HX_kArrowHeight + HX_kArrowCurvature, yMax)];
    }
    ...
    //// Shadow Declarations
    UIColor* shadow = [UIColor colorWithWhite:0.0f alpha:HX_kShadowAlpha];
    CGSize shadowOffset = CGSizeMake(0, 1);
    CGFloat shadowBlurRadius = HX_kShadowBlur;
    
    //// Gradient Declarations
    NSArray* gradientColors = [NSArray arrayWithObjects:
                               (id)HX_kGradientTopColor.CGColor,
                               (id)HX_kGradientBottomColor.CGColor, nil];
    ...
    float bottomOffset = (above ? HX_kArrowHeight : 0.f);
    float topOffset = (!above ? HX_kArrowHeight : 0.f);
    ...
    if (HX_kDrawTitleGradient) {
        ...
        if (titleView != nil) {
            titleBGHeight = HX_kBoxPadding*2.f + titleView.frame.size.height;
        }
        ...
        if (titleBGHeight > 0.f) {
            ...
            //In this case, the arrow is located between LT2 and RT1
            if (!above) {
                [titleBGPath addLineToPoint:CGPointMake(arrowPoint.x - HX_kArrowHeight, yMin)];//left side
                [titleBGPath addCurveToPoint:arrowPoint controlPoint1:CGPointMake(arrowPoint.x - HX_kArrowHeight + HX_kArrowCurvature, yMin) controlPoint2:arrowPoint];//actual arrow point
                [titleBGPath addCurveToPoint:CGPointMake(arrowPoint.x + HX_kArrowHeight, yMin) controlPoint1:arrowPoint controlPoint2:CGPointMake(arrowPoint.x + HX_kArrowHeight - HX_kArrowCurvature, yMin)];//right side
            }
            ...
            //// Gradient Declarations
            NSArray* gradientColors = [NSArray arrayWithObjects:
                                       (id)HX_kGradientTitleTopColor.CGColor,
                                       (id)HX_kGradientTitleBottomColor.CGColor, nil];
            CGFloat gradientLocations[] = {0, 1};
            CGGradientRef gradient = CGGradientCreateWithColors(colorSpace, (CFTYPECAST(CFArrayRef)gradientColors), gradientLocations);
    ...
    {
        if (HX_kShowDividersBetweenViews && showDividerRects) {
    ...
    if (HX_kDrawBorder) {
        [HX_kBorderColor setStroke];
        popoverPath.lineWidth = HX_kBorderWidth;
        [popoverPath stroke];
    }
