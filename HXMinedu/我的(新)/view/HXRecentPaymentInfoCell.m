...
@interface HXRecentPaymentInfoCell ()
...
@implementation HXRecentPaymentInfoCell
...
-(void)setZzyAndZcpModel:(HXZzyAndZcpModel *)zzyAndZcpModel{
  
    _zzyAndZcpModel = zzyAndZcpModel;
    
    self.typeImageView.image = [UIImage imageNamed:@"biaozhun"];
    self.typeLabel.text = @"标准 ";
    [self.middleContainerView.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [obj removeFromSuperview];
        obj = nil;
    }];
    ...
    UIView *lastView = self.middleContainerView;
    ///创建条目
    for (int i = 0; i < zzyAndZcpModel.stopStudyByZzyAndZcpNewMajorFeeInfoList.count; i++) {
        HXPaymentDetailModel *paymentDetailModel = zzyAndZcpModel.stopStudyByZzyAndZcpNewMajorFeeInfoList[i];
        UIView *itemView = [[UIView alloc] init];
        itemView.backgroundColor = [UIColor clearColor];
        [self.middleContainerView addSubview:itemView];
        itemView.sd_layout
    ...
    self.middleContainerView.sd_layout.heightIs(40*zzyAndZcpModel.stopStudyByZzyAndZcpNewMajorFeeInfoList.count);
    [self.middleContainerView updateLayout];
