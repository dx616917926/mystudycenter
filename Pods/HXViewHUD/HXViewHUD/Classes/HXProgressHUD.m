...
#define HXMainThreadAssert() NSAssert([NSThread isMainThread], @"HXProgressHUD needs to be accessed on the main thread.");

CGFloat const HXProgressMaxOffset = 1000000.f;

static const CGFloat HXDefaultPadding = 10.f;
static const CGFloat HXDefaultLabelFontSize = 16.f;
static const CGFloat HXDefaultDetailsLabelFontSize = 12.f;


@interface HXProgressHUD () {
    // Deprecated
...
@interface HXProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    HXProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    HXProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (HXProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (HXProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = HXProgressHUDAnimationFade;
    _mode = HXProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(HXProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == HXProgressHUDAnimationZoom) {
        type = animatingIn ? HXProgressHUDAnimationZoomIn : HXProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == HXProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == HXProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == HXProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == HXProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    HXProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<HXProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    HXBackgroundView *backgroundView = [[HXBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = HXProgressHUDBackgroundStyleSolidColor;
    ...
    HXBackgroundView *bezelView = [HXBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:HXDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:HXDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [HXProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:HXDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[HXRoundProgressView class]];

    HXProgressHUDMode mode = self.mode;
    if (mode == HXProgressHUDModeIndeterminate) {
    ...
    else if (mode == HXProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[HXBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == HXProgressHUDModeDeterminate || mode == HXProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[HXRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == HXProgressHUDModeAnnularDeterminate) {
            [(HXRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == HXProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == HXProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        UIActivityIndicatorView *appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[HXProgressHUD class], nil];
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[HXRoundProgressView class]]) {
        HXRoundProgressView *appearance = [HXRoundProgressView appearanceWhenContainedIn:[HXProgressHUD class], nil];
        if (appearance.progressTintColor == nil) {
            ((HXRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((HXRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[HXBarProgressView class]]) {
        HXBarProgressView *appearance = [HXBarProgressView appearanceWhenContainedIn:[HXProgressHUD class], nil];
        if (appearance.progressColor == nil) {
            ((HXBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((HXBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    HXBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? HXDefaultPadding : 0.f;
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == HXProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation HXRoundProgressView
...
@implementation HXBarProgressView
...
@interface HXBackgroundView ()

@end


@implementation HXBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        _style = HXProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(HXProgressHUDBackgroundStyle)style {
...
@implementation HXProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [HXProgressHUD allHUDsForView:view];
    for (HXProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable HXProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    HXBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == HXProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    HXMainThreadAssert();
    self.backgroundView.style = HXProgressHUDBackgroundStyleSolidColor;
...
@implementation HXProgressHUDRoundedButton
